...
int main(int argc,char **argv)
{
    ...
    servaddr.sin_family=AF_INET;//置地址族为AF_NET
    servaddr.sin_port=htons(13);//端口号为13，网络套接字地址结构中IP地址和端口号这两个成员必须使用特定格式，为此我们调用库函数htons("主机到网络短整数"）去转换二进制端口号
    if(inet_pton(AF_INET,argv[1],&servaddr.sin_addr)<=0)//IP地址为第一个命令行参数的值（agrv[1]）。调用库函数inet_pton("呈现形式到数值")去把ASCII命令行参数（例如运行本例字所用的206.168.1112.96)转换为合适的格式。
