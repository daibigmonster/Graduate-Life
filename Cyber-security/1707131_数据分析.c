...
int main()
{
   ...
   //  cout<<inet_ntoa(*((struct in_addr *)&(ip_and_port.saddr)))
    cout<<inet_ntoa(*((struct in_addr *)&(All_data[1202].src_addr)))<<" ";
    cout<<inet_ntoa(*((struct in_addr *)&(All_data[1202].dist_addr)))<<endl;
    fclose(fp);
    free(pkt_header);
    free(iph);
    free(res);
