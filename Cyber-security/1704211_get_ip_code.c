/*在分析以太网协议的时候，如果以太网协议的类型是0x800,就表示上层协议类型是IP协议。所以要捕获和分析IP数据包，然后在分析IP协议
  同样，还是使用函数pcap_loop()来进行数据包捕获，回调函数的功能是分析以太网数据包，然后根据分析的结果，在调用分析IP协议的函数。实现分析IP协议的函数定义位ip_protocol_packet_callback()*/
#include <pcap.h>
#include <stdio.h>
#include<stdlib.h>
#include <arpa/inet.h>
