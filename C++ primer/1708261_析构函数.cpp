...
可以将它编写为不执行任何操作的函数
Stock::~Stock(){}
什么时候应该调用析构函数呢？这由编译器决定，通常不应在代码中显式第调用析构函数。如果创建的是静态存储来对象，则其析构函数将在程序结束时自动被调用。如果创建的是自动存储类对象，则其析构函数将在程序执行完代码块是自动被调用。如果对象是通过new创建的，则它将驻留在栈内存或自由存取中，当使用delete来释放内存是，其析构函数将自动被调用。最后程序可以创建临时对象来完成特定的操作，在这种情况下：
