...
from Testtwo.items import UserItem
...
class ZhihuSpider(Spider):
    ...
    def start_requests(self):
        ...
        yield Request(self.followers_url.format(user=self.start_user, include=self.followers_query,
    ...
    def parse_follows(self, response):
        ...
        if 'data' in results.keys():
            for result in results.get('data'):
                yield Request(self.user_url.format(user = \
                    result.get('url_token'),include = self.user_query),
                               self.parse_user)
